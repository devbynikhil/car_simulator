...
export class Achievements {
  ...
  setGlobalProgress() {
    ...
    this.globalProgress.element =
      this.menu.instance.contentElement.querySelector(".js-global-progress");
    this.globalProgress.currentElement =
      this.globalProgress.element.querySelector(".js-current");
    this.globalProgress.totalElement =
      this.globalProgress.element.querySelector(".js-total");
    this.globalProgress.timeElement =
      this.globalProgress.element.querySelector(".js-time");
    ...
    this.globalProgress.update = () => {
      ...
      this.globalProgress.ratioUniform.value =
        this.globalProgress.achievedCount / this.globalProgress.totalCount;

      this.globalProgress.totalElement.textContent =
        this.globalProgress.totalCount;
      this.globalProgress.currentElement.textContent =
        this.globalProgress.achievedCount;
    ...
    this.globalProgress.reset = () => {
      ...
      this.globalProgress.timeEnd = 0;
      localStorage.setItem(
        "achievementsTimeStart",
        this.globalProgress.timeStart,
      );
  ...
  setRewards() {
    this.rewards = {};
    this.rewards.elements =
      this.menu.instance.contentElement.querySelectorAll(".js-reward");
    ...
    for (const rewardElement of this.rewards.elements) {
      ...
      item.threshold = Math.round(
        (i / (this.rewards.count - 1)) * this.globalProgress.totalCount,
      );
      item.locked = true;
      item.tooltipTextElement =
        item.element.querySelector(".js-tooltip strong");
  ...
  createGroup(name) {
    ...
    group.setProgress = (_progress, _silent = false) => {
      let oldProgress =
        group.progress instanceof Set ? group.progress.size : group.progress;
  ...
  setItems() {
    const itemsElement =
      this.menu.instance.contentElement.querySelector(".js-items");
    ...
    for (const [name, title, description, total] of achievementsData) {
      ...
      achievement.progressCurrentElement =
        achievement.itemElement.querySelector(".current");
      achievement.barFillElement =
        achievement.itemElement.querySelector(".bar .fill");
  ...
  setReset() {
    const button =
      this.menu.instance.contentElement.querySelector(".js-button-reset");
