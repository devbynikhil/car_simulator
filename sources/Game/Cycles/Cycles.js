...
export class Cycles {
  constructor(
    ...
    this.newAbsoluteProgressBinding = this.game.debug.addManualBinding(
      ...
      () => {
        return forcedProgress !== null
          ? forcedProgress
          : new Date().getTime() / 1000 / this.duration;
      },
      manual,
    );
  ...
  update(firstFrame = false) {
    ...
    this.punctualEvents.forEach((punctualEvent) => {
      if (
        newProgress >= punctualEvent.progress &&
        this.progress < punctualEvent.progress
      ) {
        if (firstFrame)
    ...
    this.intervalEvents.forEach((intervalEvent) => {
      const inInterval =
        newProgress > intervalEvent.startProgress &&
        newProgress < intervalEvent.endProgress;
    ...
    if (this.override.strength > 0 && this.override.progress !== null)
      this.progress = lerp(
        this.progress,
        this.override.progress,
        this.override.strength,
      );
    ...
    for (const keyframe of this.keyframesList) {
      ...
      for (const key in this.properties) {
        ...
        if (property.type === "color")
          property.value.lerpColors(
            stepPrevious.properties[key],
            stepNext.properties[key],
            mixRatio,
          );
