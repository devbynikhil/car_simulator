...
export class Materials {
  constructor() {
    ...
    this.createGradient(
      ...
      "#ff3a3a",
      "#721551",
      this.debugPanel?.addFolder({ title: "redGradient" }),
    );
  }
  ...
  setGradient() {
    ...
    if (this.game.debug.active) {
      ...
      for (const color of colors) {
        debugPanel
          .addBinding(color, "stop", { min: 0, max: 1, step: 0.001 })
          .on("change", update);
        debugPanel
          .addBinding(color, "value", { view: "color" })
          .on("change", update);
      }
    }
  }
  ...
  createEmissive(
    ...
    if (this.game.debug.active && debugPanel) {
      ...
      debugPanel.addBinding(intensity, "value", {
        min: 0,
        max: 10,
        step: 0.01,
      });
  ...
  createEmissiveGradient(
    ...
    const outputNode = Fn(() => {
      ...
      outputColor.assign(
        MeshDefaultMaterial.revealDiscardNodeBuilder(this.game, outputColor),
      );
  ...
  setPreviews() {
      ...
      this.list.forEach((material, name) => {
        if (!this.previews.list.has(name)) {
          ...
          const maxLength = Math.max(
            pureColor.r,
            Math.max(pureColor.g, pureColor.b),
          );
          if (maxLength > 1)
            pureColor.set(
              pureColor.r / maxLength,
              pureColor.g / maxLength,
              pureColor.b / maxLength,
            );
    ...
    // Debug
    if (this.game.debug.active) {
      this.debugPanel.addBinding(this.previews.group, "visible", {
        label: "previewsVisibile",
      });
    }
  }
  ...
  createFromMaterial(baseMaterial) {
    if (
      baseMaterial.isMeshLambertNodeMaterial ||
      baseMaterial.isMeshStandardMaterial
    ) {
      // Shadow
      // material.shadowSide = THREE.BackSide
  ...
  copy(baseMaterial, targetMaterial) {
    ...
    for (const property of properties) {
      if (
        typeof baseMaterial[property] !== "undefined" &&
        typeof targetMaterial[property] !== "undefined"
      )
  ...
  updateObject(mesh) {
    mesh.traverse((child) => {
      if (child.isMesh) {
        if (
          typeof child.material.userData.prevent === "undefined" ||
          !child.material.userData.prevent
        )
          child.material = this.getFromName(
            child.material.name,
            child.material,
          );
      }
