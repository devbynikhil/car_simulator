  ...
  setAmbiants() {
    const getRandomDirection = (distance = 30) => {
      return new THREE.Vector3(
        this.game.view.focusPoint.position.x +
          (Math.random() < 0.5 ? distance : -distance),
    ...
    // Birds tweets
    {
      ...
      const tryPlay = () => {
        // Chance to trigger
        if (
          !this.game.dayCycles.intervalEvents.get("night").inInterval &&
          Math.random() < 0.5
        ) {
    ...
    // Crickets
    {
      const sound = this.register({
        ...
        autoplay: true,
        loop: true,
        volume: this.game.dayCycles.intervalEvents.get("night").inInterval
          ? 0.65
          : 0,
      });
    ...
    this.register({
      ...
      onPlaying: (item) => {
        ...
        const targetVolume =
          Math.max(0, this.game.weather.snow.value) * 0.35 * sine;

        const easing = targetVolume > item.volume ? 0.005 : 0.05;
        item.volume +=
          (targetVolume - item.volume) * this.game.ticker.deltaScaled * easing;
      },
    });
    ...
    this.register({
      ...
      volume: 0,
      onPlaying: (item) => {
        const snowAttenuation = remapClamp(
        ...
        const rainVolume = remapClamp(
          this.game.weather.rain.value,
          0.1,
          0.6,
          0,
          1,
    ...
    this.register({
      ...
      onPlaying: (item) => {
        item.volume =
          Math.pow(remapClamp(this.game.weather.wind.value, 0.3, 1, 0, 1), 3) *
          0.7;
      },
    });
    ...
    this.register({
      ...
      onPlaying: (item) => {
        ...
        item.volume =
          Math.pow(remapClamp(distanceToSide, 0, 40, 1, 0.1), 2) * 0.7;
      },
    });
    ...
    // Oven fire (Project Area + Cookie Area)
    {
      ...
      if (this.game.world.areas?.cookie)
        positions.push(
          this.game.world.areas.cookie.references.items.get("spawner")[0]
            .position,
        );
      if (this.game.world.areas?.projects)
        positions.push(
          this.game.world.areas.projects.references.items.get("oven")[0]
            .position,
        );
    ...
    // Campfire (Lab Area + Bonfire Area)
    {
      ...
      if (this.game.world.areas?.lab)
        positions.push(
          this.game.world.areas.lab.references.items.get("fire")[0].position,
        );
      ...
      for (const [path, baseVolume] of paths) {
        this.register({
          ...
          onPlay: (item, force, position) => {
            item.positions[0].copy(position);
            item.volume =
              baseVolume * Math.pow(remapClamp(force, 5, 20, 0, 1), 2);
            item.rate = 0.9 + Math.random() * 0.2;
          },
        });
      }
        ...
        if (item.positions && item.howl.playing()) {
          ...
          for (const position of item.positions) {
            const distance = position.distanceTo(
              this.game.view.focusPoint.position,
            );
          ...
          cameraRelativePosition.applyMatrix4(
            this.game.view.camera.matrixWorldInverse,
          );
          cameraRelativePosition.normalize();
          cameraRelativePosition.z *= 0.1;

          if (item.distanceFade) {
            distanceFadeMultiplier = remapClamp(
              closestDistance,
              0,
              item.distanceFade,
              1,
