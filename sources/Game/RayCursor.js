...
export class RayCursor {
  ...
  removeIntersect(intersect) {
    this.intersects = this.intersects.filter(
      (_intersect) => _intersect !== intersect,
    );
  }
  ...
  testIntersects(actionTrigger) {
    ...
    if (actionTrigger === "start" || actionTrigger === "change") {
      ...
      if (intersects.length) {
        const ndcPointer = new THREE.Vector2(
          (this.game.inputs.pointer.current.x / this.game.viewport.width) * 2 -
            1,
          -(
            (this.game.inputs.pointer.current.y / this.game.viewport.height) *
              2 -
            1
          ),
        );
        ...
        for (const intersect of intersects) {
          ...
          if (intersect.shape instanceof THREE.Sphere)
            isIntersecting = this.raycaster.ray.intersectsSphere(
              intersect.shape,
            );
          ...
          if (intersect.shape instanceof THREE.Plane)
            isIntersecting = this.raycaster.ray.intersectsPlane(
              intersect.shape,
            );
          if (intersect.shape instanceof THREE.Mesh)
            isIntersecting = this.raycaster.intersectObject(
              intersect.shape,
            ).length;
    ...
    // End
      ...
      const intersects = this.intersects.filter(
        (intersect) => intersect.active,
      );
  ...
  setPointerTesting() {
    this.game.inputs.addActions([
      {
        name: "rayPointer",
        categories: ["intro", "wandering", "racing", "cinematic"],
        keys: ["Pointer.any"],
      },
    ]);
