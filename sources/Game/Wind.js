...
export class Wind {
  constructor() {
    ...
    this.offsetNode = Fn(([position]) => {
      ...
      const noiseUv1 = remapedPosition.xy
        .mul(0.2)
        .add(this.direction.mul(this.localTime)).xy;
    ...
    if (this.game.debug.active) {
      ...
      this.debugPanel.addBinding(this, "timeFrequency", {
        min: 0,
        max: 1,
        step: 0.001,
      });
      this.debugPanel
        ...
        .on("change", (tweak) => {
          this.direction.value.set(
            Math.sin(tweak.value),
            Math.cos(tweak.value),
          );
        });
    }
  }
  ...
  update() {
    ...
    this.localTime.value +=
      this.game.ticker.deltaScaled * this.timeFrequency * this.strength.value;
  }
}
