...
export class Inputs {
  ...
  setGamepad() {
    ...
    this.gamepad.events.on("joystickChange", (joystick) => {
      this.change(
        `Gamepad.joystick${joystick.name.charAt(0).toUpperCase() + joystick.name.slice(1)}`,
        {
          x: joystick.x,
          y: joystick.y,
  ...
  setPointer() {
    ...
    this.pointer.events.on("down", () => {
      this.updateMode(
        this.pointer.mode === Pointer.MODE_MOUSE
          ? Inputs.MODE_MOUSEKEYBOARD
          : Inputs.MODE_TOUCH,
      );
      this.start("Pointer.any", {
        x: this.pointer.current.x,
        y: this.pointer.current.y,
      });
    });
