      ...
      for (const map of mapping) {
        ...
        if (map.type === "joystick") {
          ...
          if (map.action === "axesToCircle") {
            ...
            joystick.safeX =
              remapClamp(Math.abs(joystick.x), joystickDeadZone, 1, 0, 1) *
              Math.sign(joystick.x);

            joystick.y = gamepad.axes[map.indexes[1]];
            joystick.safeY =
              remapClamp(Math.abs(joystick.y), joystickDeadZone, 1, 0, 1) *
              Math.sign(joystick.y);

            joystick.angle = Math.atan2(joystick.y, joystick.x);
            joystick.radius = Math.hypot(joystick.y, joystick.x);

            joystick.safeRadius = remapClamp(
              joystick.radius,
              joystickDeadZone,
              1,
              0,
