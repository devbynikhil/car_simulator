...
export class Pointer {
  ...
  constructor(element) {
    ...
    this.element.addEventListener("touchend", (_event) => {
      ...
      if (
        this.upcomingTouches.length === 0 ||
        this.upcomingTouches.length === 1
      )
        this.upcomingDown = false;
    });
    ...
    if (this.upcomingTouches.length >= 2) {
      ...
      for (let i = 0; i < this.upcomingTouches.length; i++) {
        for (let j = i + 1; j < this.upcomingTouches.length; j++) {
          const dX =
            this.upcomingTouches[i].clientX - this.upcomingTouches[j].clientX;
          const dY =
            this.upcomingTouches[i].clientY - this.upcomingTouches[j].clientY;
