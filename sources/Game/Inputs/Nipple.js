...
export class Nipple {
  ...
  setMeshes() {
    ...
    material.outputNode = Fn(() => {
      ...
      // Edges
      const innerEdgeSDF = abs(
        radialCoord.length().sub(this.progressRadiusLow),
      ).toVar();
      const outerEdgeSDF = abs(
        radialCoord.length().sub(this.progressRadiusHigh),
      ).toVar();

      const innerEdgeFill = step(innerEdgeSDF, this.edgesThickness / 2).toVar();
      const innerEdgeOutline = step(
        innerEdgeSDF,
      ...
      const progressSDF = radialCoord
        ...
        .sub(
          this.uniforms.progress.mul(
            this.progressRadiusHigh -
              this.progressRadiusLow -
              this.outlineThickness / 2,
          ),
        )
      ...
      const progressOutline = step(progressSDF, this.outlineThickness / 4).mul(
        directionAngle,
      );
  ...
  update() {
    if (this.active || this.animated) {
      ...
      if (this.forward) {
        this.uniforms.progressStartAngle.value = Math.min(
          0,
          this.smallestAngle,
        );
