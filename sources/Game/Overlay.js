...
export class Overlay {
  constructor() {
    ...
    material.outputNode = Fn(() => {
      ...
      const patternMask = texture(
        this.game.resources.overlayPatternTexture,
        patternUv,
      )
        .a.remap(0, 0.68, 0, 1)
        .toVar();
      ...
      // Gradient
      const colorHash = texture(
        this.game.noises.hash,
        viewportCoordinate.div(this.game.noises.resolution),
      )
        .r.sub(0.5)
        .mul(0.2);
    ...
    if (this.game.debug.active) {
      ...
      debugPanel
        .addBinding(this.progress, "value", {
          label: "progress",
          min: 0,
          max: 1,
          step: 0.001,
      ...
      debugPanel.addBinding(this.patternSize, "value", {
        label: "patternSize",
        min: 0,
        max: 500,
        step: 1,
      });
      debugPanel.addBinding(this.strokeSize, "value", {
        label: "strokeSize",
        min: 0,
        max: 50,
        step: 1,
      });
      debugPanel.addBinding(this.inverted, "value", {
        label: "inverted",
        min: 0,
        max: 1,
        step: 1,
