...
export class Title {
  ...
  update() {
    this.offset +=
      this.game.physicalVehicle.forwardSpeed * this.game.ticker.deltaScaled;
    ...
    if (delta > this.interval && offsetRounded !== this.offsetRounded) {
      ...
      for (let i = 0; i < line.length - 1; i++) {
        ...
        if (leftItem.position + leftItem.size > rightItem.position) {
          const overlapLength =
            leftItem.position + leftItem.size - rightItem.position;
      ...
      document.title = "NK Drive" + title.join("");
    }
  }
}
