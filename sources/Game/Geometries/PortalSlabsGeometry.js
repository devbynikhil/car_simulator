...
export class PortalSlabsGeometry extends THREE.BufferGeometry {
  constructor(size = 1, columnsCount = 5) {
    ...
    for (let x = 0; x < columnsCount; x++) {
      for (let z = 0; z < columnsCount; z++) {
        ...
        for (let i = 0; i < verticesCount; i++) {
          positions[(slabIndex * verticesCount + i) * 3 + 0] =
            (slabPositions[i * 3 + 0] * size) / 2 + slabX;
          positions[(slabIndex * verticesCount + i) * 3 + 1] =
            slabPositions[i * 3 + 1];
          positions[(slabIndex * verticesCount + i) * 3 + 2] =
            (slabPositions[i * 3 + 2] * size) / 2 + slabZ;
          ...
          distanceToCenters[slabIndex * verticesCount + i] = Math.hypot(
            slabX,
            slabZ,
          );
        }
        ...
        for (let i = 0; i < indicesCount; i++) {
          indices[slabIndex * indicesCount + i] =
            slabIndex * verticesCount + slabIndices[i + 0];
        }
      }
    }
    ...
    this.setAttribute(
      "distanceToCenter",
      new THREE.Float32BufferAttribute(distanceToCenters, 1),
    );
    this.setIndex(new THREE.Uint16BufferAttribute(indices, 1));
  }
}
