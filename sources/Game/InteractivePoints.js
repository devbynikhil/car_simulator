...
export class InteractivePoints {
  ...
  setKeyIcon() {
    ...
    material.outputNode = iconOutput(
      texture(
        this.game.resources.interactivePointsKeyIconEnterTexture,
    ...
    this.game.inputs.gamepad.events.on("typeChange", () => {
      if (this.game.inputs.mode === Inputs.MODE_GAMEPAD) {
        ...
        material.outputNode = iconOutput(
          texture(iconTexture, vec2(uv().x, uv().y.oneMinus())).r,
        );
  ...
  setInputs() {
    this.game.inputs.events.on("interact", (action) => {
      if (
        action.active &&
        this.activeItem &&
        this.activeItem.state === InteractivePoints.STATE_OPEN
      ) {
  ...
  create(
    ...
    const width =
      Math.ceil(textSize.width) + textPaddingLeft + textPaddingRight + 2;
    canvas.width = width;
    ...
    // Material
    const labelMaterial = new THREE.MeshLambertNodeMaterial({
      transparent: true,
      depthTest: true,
    });
    ...
    labelMaterial.outputNode = Fn(() => {
      ...
      // Fogged back color
      const foggedBackColor = this.game.fog.strength.mix(
        this.backColor,
        this.game.fog.color,
      );
    ...
    label.position.x =
      align === InteractivePoints.ALIGN_LEFT ? 0 : -label.scale.x;
    label.visible = false;
    group.add(label);
     ...
     */
    // Material
    const diamondMaterial = new THREE.MeshLambertNodeMaterial({
      transparent: true,
      depthTest: true,
    });
    ...
    item.conceal = () => {
      ...
      const ease =
        previousState === InteractivePoints.STATE_HIDDEN
          ? "power2.out"
          : "back.in(4.5)";
    ...
    if (this.game.debug.active) {
      ...
      this.debugPanel.addBinding(threshold, "value", {
        label: "threshold",
        min: 0,
        max: 1,
        step: 0.001,
      });
      this.debugPanel.addBinding(lineThickness, "value", {
        label: "lineThickness",
        min: 0,
        max: 1,
        step: 0.001,
      });
      this.debugPanel.addBinding(lineOffset, "value", {
        label: "lineOffset",
        min: 0,
        max: 1,
        step: 0.001,
      });
      this.debugPanel.addBinding(labelOffset, "value", {
        label: "labelOffset",
        min: 0,
        max: 1,
        step: 0.001,
