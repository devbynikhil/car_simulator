...
export class BlackFriday {
  ...
  setMobile() {
    ...
    if (this.isMobile) {
      this.element.querySelector(".no-mobile-container").style.display =
        "block";
  ...
  setOutro() {
    ...
    this.outro.show = () => {
      ...
      this.outro.linkElement.href = this.outro.linkElement.href.replace(
        "XXX",
        this.fragments.code,
      );
  ...
  setFragments() {
    ...
    this.fragments.fragmentElements =
      this.fragments.containerElement.querySelectorAll(".fragment");
    this.fragments.closest = null;

    this.fragments.code = "bf2024";
    this.fragments.list = [
      {
        position: this.game.resources.fragments.scene.children[0].position,
        character: this.fragments.code[0],
      },
      {
        position: this.game.resources.fragments.scene.children[1].position,
        character: this.fragments.code[1],
      },
      {
        position: this.game.resources.fragments.scene.children[2].position,
        character: this.fragments.code[2],
      },
      {
        position: this.game.resources.fragments.scene.children[3].position,
        character: this.fragments.code[3],
      },
      {
        position: this.game.resources.fragments.scene.children[4].position,
        character: this.fragments.code[4],
      },
      {
        position: this.game.resources.fragments.scene.children[5].position,
        character: this.fragments.code[5],
      },
    ];
    ...
    this.fragments.getClosest = () => {
      ...
      for (const _fragment of this.fragments.list) {
        if (!_fragment.caught) {
          _fragment.distance = _fragment.position.distanceTo(
            this.game.vehicle.position,
          );
    ...
    this.fragments.tryCatch = (_fragment) => {
      if (
        _fragment.distance < this.fragments.catchDistance &&
        !_fragment.caught
      )
    ...
    this.fragments.testOver = () => {
      this.fragments.allCaught = this.fragments.list.reduce(
