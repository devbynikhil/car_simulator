...
export class Map {
  ...
  setInputs() {
    ...
    this.game.inputs.addActions([
      {
        name: "map",
        categories: ["modal", "menu", "wandering"],
        keys: ["Keyboard.m", "Keyboard.KeyM"],
      },
    ]);
  ...
  worldToMap(coordinates) {
    let x = coordinates.x;
    let y =
      typeof coordinates.z !== "undefined" ? coordinates.z : coordinates.y;
  ...
  update() {
    ...
    if (
      playerRoundedX !== this.player.roundedPosition.x ||
      playerRoundedY !== this.player.roundedPosition.y
    ) {
