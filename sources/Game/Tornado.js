...
export class Tornado {
  constructor() {
    ...
    this.achievementAchieved =
      this.game.achievements.groups.get("cataclysm")?.items[0].achieved;
  ...
  setPath() {
    ...
    const children = [
      ...this.game.resources.tornadoPathReferencesModel.scene.children,
    ];
    children.sort((a, b) => {
  ...
  setPreviews() {
    ...
    this.previews.target = new THREE.Mesh(
      boxGeometry,
      new THREE.MeshBasicNodeMaterial({ color: "#00ff00" }),
    );
    this.previews.target.scale.y = 10;
    this.game.scene.add(this.previews.target);

    this.previews.eased = new THREE.Mesh(
      boxGeometry,
      new THREE.MeshBasicNodeMaterial({ color: "#00ffff" }),
    );
  ...
  update() {
    ...
    // Physics vehicle
    const toTornado = this.position
      .clone()
      .sub(this.game.physicalVehicle.position);
    ...
    force.setLength(
      strength * this.game.ticker.deltaScaled * this.strength * 30,
    );
