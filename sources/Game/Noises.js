...
const voronoiNode = /*#__PURE__*/ Fn(([uv_immutable, repeat_immutable]) => {
  ...
  Loop({ start: int(-1), end: int(1), name: "y", condition: "<=" }, ({ y }) => {
    Loop(
      { start: int(-1), end: int(1), name: "x", condition: "<=" },
      ({ x }) => {
...
export const modulo = /*#__PURE__*/ Fn(
  ([divident_immutable, divisor_immutable]) => {
...
export const random = /*#__PURE__*/ Fn(([value_immutable]) => {
  ...
  value.assign(
    vec2(dot(value, vec2(127.1, 311.7)), dot(value, vec2(269.5, 183.3))),
  );
...
export const perlinNode = /*#__PURE__*/ Fn(
  ([uv_immutable, cell_amount_immutable, period_immutable]) => {
    ...
    const lowerLeftDirection = vec2(
      random(vec2(cellsMinimum.x, cellsMinimum.y)),
    ).toVar();
    const lowerRightDirection = vec2(
      random(vec2(cellsMaximum.x, cellsMinimum.y)),
    ).toVar();
    const upperLeftDirection = vec2(
      random(vec2(cellsMinimum.x, cellsMaximum.y)),
    ).toVar();
    const upperRightDirection = vec2(
      random(vec2(cellsMaximum.x, cellsMaximum.y)),
    ).toVar();
