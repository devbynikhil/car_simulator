    ...
    this.outputNode = Fn(() => {
      ...
      if (this.hasLightBounce) {
        const bounceOrientation = reorientedNormal
          .dot(vec3(0, -1, 0))
          .smoothstep(
            this.game.lighting.lightBounceEdgeLow,
            this.game.lighting.lightBounceEdgeHigh,
          );
        ...
        outputColor.assign(
          mix(
            ...
            bounceOrientation
              .mul(bounceDistance)
              .mul(this.game.lighting.lightBounceMultiplier),
          ),
        );
      }
      ...
      if (this.hasWater) {
        ...
        outputColor.assign(
          nearWaterSurface.select(outputColor, color("#ffffff")),
        );
      ...
      outputColor.mulAssign(
        this.game.lighting.colorUniform.mul(
          this.game.lighting.intensityUniform,
        ),
      );
      ...
      if (this.hasCoreShadows)
        coreShadowMix = reorientedNormal
          ...
          .smoothstep(
            this.game.lighting.coreShadowEdgeHigh,
            this.game.lighting.coreShadowEdgeLow,
          );
      ...
      if (this.hasFog)
        outputColor.assign(
          this.game.fog.strength.mix(outputColor, this.game.fog.color),
        );
