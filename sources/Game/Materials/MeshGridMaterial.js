...
const toGrid = Fn(([uv, scale, thickness, offset, cross]) => {
  ...
  const crossGrid = step(
    referenceUv.fract().sub(0.5).abs(),
    cross.oneMinus().mul(0.5),
  );
  const crossMask = mix(crossGrid.x, 1, crossGrid.y).oneMinus();
  const grid = step(
    referenceUv.sub(0.5).fract().sub(0.5).abs().mul(2),
    thickness,
  ).mul(crossMask);
...
const toAntialiasedGrid = Fn(
  ([uv, scale, thickness, offset, cross, derivateMask]) => {
    ...
    grid2 = mix(grid2, lineWidth, clamp(uvDeriv.mul(2).sub(1), 0, 1)).mul(
      crossMask,
    );
