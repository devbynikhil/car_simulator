...
export class View {
  ...
  constructor(idealRatio = 1920 / 1080) {
    ...
    this.ratioOverflow =
      Math.max(1, this.idealRatio / this.game.viewport.ratio) - 1;
  ...
  toggleMode() {
    this.setMode(
      this.mode === View.MODE_FREE ? View.MODE_DEFAULT : View.MODE_FREE,
    );
  }
  ...
  setMode(mode) {
    ...
    this.freeMode.setTarget(
      this.focusPoint.position.x,
      this.focusPoint.position.y,
      this.focusPoint.position.z,
    );
    this.freeMode.setPosition(
  ...
  setFocusPoint() {
    ...
    if (this.game.debug.active) {
      ...
      zoomDebugPanel.addBinding(this.focusPoint.magnet, "multiplier", {
        min: 0,
        max: 1,
        step: 0.0001,
      });
  ...
  setOptimalArea() {
    ...
    this.optimalArea.floorPlane = new THREE.Plane(
    ...
    this.optimalArea.update = () => {
      ...
      this.optimalArea.raycaster.setFromCamera(
        new THREE.Vector2(1, -1),
        this.defaultCamera,
      );
      this.optimalArea.raycaster.ray.intersectPlane(
        this.optimalArea.floorPlane,
        this.optimalArea.nearPosition,
      );
      ...
      const centerA = this.optimalArea.nearPosition
        .clone()
        .lerp(this.optimalArea.farPosition, 0.5);
      ...
      const centerB = this.optimalArea.nearPosition
        .clone()
        .lerp(this.optimalArea.farPosition, 0.5);
      ...
      // Radius
      this.optimalArea.radius = this.optimalArea.basePosition.distanceTo(
        this.optimalArea.farPosition,
      );
      ...
      this.optimalArea.nearDistance = this.defaultCamera.position.distanceTo(
        this.optimalArea.nearPosition,
      );
      this.optimalArea.farDistance = this.defaultCamera.position.distanceTo(
        this.optimalArea.farPosition,
      );
  ...
  setZoom() {
    ...
    if (this.game.debug.active) {
      ...
      zoomDebugPanel.addBinding(this.zoom, "speedAmplitude", {
        min: 0,
        max: 1,
        step: 0.001,
      });
      zoomDebugPanel.addBinding(this.zoom, "speedEdge", {
        min: 0,
        max: 100,
        step: 0.001,
      });
      zoomDebugPanel.addBinding(this.zoom, "sensitivity", {
        min: 0,
        max: 0.5,
        step: 0.0001,
      });
  ...
  setSpherical() {
    ...
    if (this.game.debug.active) {
      ...
      sphericalDebugPanel.addBinding(this.spherical, "phi", {
        min: 0,
        max: Math.PI * 0.5,
        step: 0.001,
      });
      sphericalDebugPanel.addBinding(this.spherical, "theta", {
        min: -Math.PI,
        max: Math.PI,
        step: 0.001,
      });
      sphericalDebugPanel.addBinding(this.spherical.radius, "edges", {
        min: 0,
        max: 100,
        step: 0.001,
      });
  ...
  setRoll() {
    ...
    this.roll.kick = (strength = 1) => {
      this.roll.speed =
        strength * this.roll.kickStrength * (Math.random() < 0.5 ? -1 : 1);
    };
    ...
    if (this.game.debug.active) {
      ...
      rollDebugPanel.addBinding(this.roll, "damping", {
        min: 0,
        max: 20,
        step: 0.1,
      });
      rollDebugPanel.addBinding(this.roll, "pullStrength", {
        min: 0,
        max: 400,
        step: 0.1,
      });
      rollDebugPanel.addBinding(this.roll, "kickStrength", {
        min: 0,
        max: 10,
        step: 0.1,
      });
  ...
  setSpeedLines() {
    ...
    for (let i = 0; i < linesCount; i++) {
      ...
      const vertexLeft = vertexMiddle
        .clone()
        .rotateAround(new THREE.Vector2(), thickness);
      const vertexRight = vertexMiddle
        .clone()
        .rotateAround(new THREE.Vector2(), -thickness);
    ...
    this.speedLines.geometry.setAttribute(
      "tipness",
      new THREE.Float32BufferAttribute(tipnessArray, 1),
    );
    ...
    this.speedLines.material.vertexNode = Fn(() => {
      ...
      const newPosition = mix(
        ...
        tipness
          .mul(osciliation)
          .mul(distance)
          .mul(this.speedLines.smoothedStrength),
      );
    ...
    this.speedLines.mesh = new THREE.Mesh(
      this.speedLines.geometry,
      this.speedLines.material,
    );
    ...
    if (this.game.debug.active) {
      ...
      folder.addBinding(this.speedLines, "strength", {
        label: "strength",
        min: 0,
        max: 1,
        step: 0.001,
      });
      folder.addBinding(this.speedLines.speed, "value", {
        label: "speed",
        min: 0,
        max: 100,
        step: 0.001,
  ...
  resize() {
    ...
    this.defaultCamera.aspect =
      this.game.viewport.width / this.game.viewport.height;
    this.defaultCamera.updateProjectionMatrix();

    this.freeCamera.aspect =
      this.game.viewport.width / this.game.viewport.height;
    ...
    this.game.inputs.events.on("viewMapPointer", (action) => {
      if (this.mode === View.MODE_DEFAULT) {
        ...
        if (action.active) {
          ...
          // Pinch
          this.zoom.baseRatio +=
            this.game.inputs.pointer.pinch.distanceDelta * 0.005;
          this.zoom.baseRatio = clamp(this.zoom.baseRatio, 0, 1);
        }
      }
    });
  ...
  update() {
    ...
    if (this.focusPoint.magnet.active) {
      ...
      const magnetStrength =
        distanceToMagnet * this.focusPoint.magnet.multiplier;
      this.focusPoint.position.x +=
        magnetStrength * magnetDelta.x * this.game.ticker.delta;
      this.focusPoint.position.z +=
        magnetStrength * magnetDelta.z * this.game.ticker.delta;
    ...
    const easing = remap(
      this.focusPoint.easing,
      0,
      1,
      1,
      this.game.ticker.delta * 10,
    );
    ...
    const smoothFocusPointDelta = newSmoothFocusPoint
      .clone()
      .sub(this.focusPoint.smoothedPosition);
    const focusPointSpeed =
      Math.hypot(smoothFocusPointDelta.x, smoothFocusPointDelta.z) /
      this.game.ticker.delta;
    ...
    if (this.mode === View.MODE_DEFAULT) {
      ...
      const zoomSpeedRatio = smoothstep(
        focusPointSpeed,
        this.zoom.speedEdge.min,
        this.zoom.speedEdge.max,
      );
      this.zoom.ratio = this.zoom.baseRatio;
    ...
    this.roll.velocity =
      -this.roll.value * this.roll.pullStrength * this.game.ticker.deltaScaled;
    ...
    if (this.cinematic.progress > 0) {
      ...
      this.defaultCamera.position.lerp(
        this.cinematic.dummy.position,
        this.cinematic.progress,
      );
      this.defaultCamera.quaternion.slerp(
        this.cinematic.dummy.quaternion,
        this.cinematic.progress,
      );
    }
    ...
    this.optimalArea.position
      ...
      .add(
        new THREE.Vector3(
          this.focusPoint.smoothedPosition.x,
          0,
          this.focusPoint.smoothedPosition.z,
        ),
      );
