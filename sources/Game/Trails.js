...
export class Trails {
  constructor() {
    ...
    if (this.game.debug.active) {
      ...
      this.debugPanel.addBinding(this, "decay", {
        min: 0,
        max: 1,
        step: 0.001,
      });
  ...
  create() {
    ...
    material.positionNode = Fn(() => {
      ...
      // Direction
      const nextPosition = texture(
        item.dataTexture,
        vec2(ratio.add(this.texel), 0.5),
      ).xyz;
      ...
      // Normal
      customNormal.assign(
        modelViewMatrix.mul(vec4(rotationMatrix.mul(attribute("normal")), 0)),
      );
    ...
    material.outputNode = Fn(() => {
      ...
      const baseColor = texture(
        this.game.materials.gradientTexture,
        gradientUv,
      ).rgb.mul(this.emissiveMultiplier);
  ...
  update() {
    for (const item of this.items) {
      ...
      for (let i = this.subdivisions - 1; i >= 0; i--) {
        const i4 = i * 4;
        data[i4 + 3] = Math.max(
          data[i4 + 3] - this.game.ticker.deltaScaled * this.decay,
          0,
        );
      }
