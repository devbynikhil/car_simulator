...
export class Lighting {
  constructor() {
    ...
    this.spherical = new THREE.Spherical(
      this.game.view.optimalArea.radius + this.near,
      this.phi,
      this.theta,
    );
  ...
  setNodes() {
    ...
    this.shadowColor = uniform(
      this.game.dayCycles.properties.shadowColor.value,
    );
  ...
  setHelpers() {
    ...
    if (this.game.debug.active) {
      this.debugPanel.addBinding(this.directionHelper, "visible", {
        label: "directionHelper",
      });
      this.debugPanel.addBinding(this.shadowHelper, "visible", {
        label: "shadowHelper",
      });
    }
  }
  ...
  update() {
    ...
    if (this.useDayCycles) {
      ...
      this.spherical.theta =
        this.theta +
        Math.sin(
          -(this.game.dayCycles.progress + progressOffset) * Math.PI * 2,
        ) *
          this.thetaAmplitude;
      this.spherical.phi =
        this.phi +
    ...
    // Helper
    this.directionHelper.position
      .copy(this.direction)
      .multiplyScalar(5)
      .add(this.game.view.focusPoint.position);
    this.directionHelper.lookAt(this.game.view.focusPoint.position);

    // Apply day cycles values
    this.colorUniform.value.copy(
      this.game.dayCycles.properties.lightColor.value,
    );
    this.intensityUniform.value =
      this.game.dayCycles.properties.lightIntensity.value;
  }
}
