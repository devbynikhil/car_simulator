...
export class Easter {
  ...
  setEggVisual() {
    ...
    // Mesh
    const egg =
      this.game.resources.easterEggVisualModel.scene.getObjectByName("egg");
     ...
     */
    // Material
    const beamsMaterial = new THREE.MeshBasicNodeMaterial({
      transparent: true,
    });

    beamsMaterial.outputNode = Fn(() => {
      const strength = uv()
        .y.add(this.game.ticker.elapsedScaledUniform.mul(0.05))
        .fract();
    ...
    // Mesh
    const beams =
      this.game.resources.easterEggVisualModel.scene.getObjectByName("beams");
  ...
  setEggs() {
    ...
    this.eggs.getClosest = () => {
      ...
      for (const egg of this.eggs.items) {
        if (!egg.caught) {
          egg.distance = egg.reference.position.distanceTo(
  ...
  update() {
    ...
    if (this.game.world.visualVehicle.antenna) {
      if (this.eggs.closest) {
        this.game.world.visualVehicle.antenna.target.copy(
          this.eggs.closest.reference.position,
        );
      } else {
