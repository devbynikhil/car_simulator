...
export class Objects {
  ...
  add(_visualDescription = null, _physicalDescription = null) {
    ...
    if (object.visual && object.physical) {
      if (
        _physicalDescription.sleeping ||
        !_physicalDescription.enabled ||
        object.physical.type === "fixed"
      ) {
        object.visual.object3D.position.copy(
          object.physical.body.translation(),
        );
    ...
    if (physical) {
      ...
      for (const _child of children) {
        ...
        if (_child.name.match(/^trimesh/i)) {
          ...
          collider.parameters = [
            _child.geometry.attributes.position.array,
            _child.geometry.index.array,
          ];
  ...
  addFromModel(_model, _visualDescription = {}, _physicalDescription = {}) {
    // Add
    return this.add(
      ...this.getFromModel(_model, _visualDescription, _physicalDescription),
    );
  }
  ...
  resetObject(object) {
    ...
    object.physical.body.setTranslation(
      object.physical.initialState.position,
      false,
    );
    ...
    if (object.visual) {
      if (
        ...
        object.visual.parent !== object.visual.object3D.parent
      )
        object.visual.parent.add(object.visual.object3D);
      object.visual.object3D.position.copy(
        object.physical.initialState.position,
      );
      object.visual.object3D.quaternion.copy(
        object.physical.initialState.rotation,
      );
    }
  }
  ...
  update() {
    ...
    this.list.forEach((_object) => {
      const position = _object.physical
        ? _object.physical.body.translation()
        : null;
      ...
      if (
        ...
        _object.visual.object3D.quaternion.copy(
          _object.physical.body.rotation(),
        );
      }
