...
export class Grass {
  constructor() {
    ...
    this.surfaceOverflow =
      Math.max(0, this.surface - this.surfaceIdeal) / this.surfaceIdeal;
  ...
  setGeometry() {
    ...
    for (let iX = 0; iX < this.subdivisions; iX++) {
      const fragmentX =
        (iX / this.subdivisions - 0.5) * this.size + this.fragmentSize * 0.5;

      for (let iZ = 0; iZ < this.subdivisions; iZ++) {
        const fragmentZ =
          (iZ / this.subdivisions - 0.5) * this.size + this.fragmentSize * 0.5;
    ...
    this.geometry.setAttribute(
      "position",
      new THREE.Float32BufferAttribute(position, 2),
    );
    this.geometry.setAttribute(
      "heightRandomness",
  ...
  setMaterial() {
    ...
    this.material.positionNode = Fn(() => {
      ...
      loopPosition.x.assign(
        mod(loopPosition.x.add(halfSize), this.sizeUniform).sub(halfSize),
      );
      loopPosition.y.assign(
        mod(loopPosition.y.add(halfSize), this.sizeUniform).sub(halfSize),
      );

      const position3 = vec3(loopPosition.x, 0, loopPosition.y).add(
        vec3(this.center.x, 0, this.center.y),
      );
      const worldPosition = modelWorldMatrix.mul(position3);
      bladePosition.assign(worldPosition.xz);

      // Height
      const heightVariation = texture(
        this.game.noises.perlin,
        bladePosition.mul(0.0321),
      ).r.add(0.5);
      ...
      const shape = vec3(
        bladeShape
          .element(vertexLoopIndex.mod(3).mul(2))
          .mul(this.bladeWidth)
          .mul(terrainDataGrass),
    ...
    if (this.game.debug.active) {
      ...
      debugPanel.addBinding(this.bladeWidth, "value", {
        label: "bladeWidth",
        min: 0,
        max: 1,
        step: 0.001,
      });
      debugPanel.addBinding(this.bladeHeight, "value", {
        label: "bladeHeight",
        min: 0,
        max: 2,
        step: 0.001,
