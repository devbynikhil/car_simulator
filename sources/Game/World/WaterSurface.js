...
export class WaterSurface {
  ...
  setNodes() {
    ...
    const splashesNode = Fn(() => {
      ...
      // Time
      const splashTimeRandom = hash(splashesVoronoi.b.mul(123456)).add(
        splashPerlin,
      );
      const splashTime = this.game.wind.localTime
        .mul(splashesTimeFrequency)
        .add(splashTimeRandom);
    ...
    // Debug
    if (this.game.debug.active) {
      this.shoreDebugPanel.addBinding(shoreEdge, "value", {
        label: "shoreEdge",
        min: 0,
        max: 0.3,
        step: 0.001,
    ...
    // Debug
    if (this.game.debug.active) {
      this.blurDebugPanel.addBinding(blurStrength, "value", {
        label: "blurStrength",
        min: 0,
        max: 0.1,
      });
