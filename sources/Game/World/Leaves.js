...
export class Leaves {
  constructor() {
    ...
    const power = Math.round(
      remap(this.game.yearCycles.properties.leaves.value, 0.25, 1, 7, 11),
    );
  ...
  setMaterial() {
    ...
    this.material.positionNode = Fn(() => {
      ...
      const rotationMultiplier = max(
        leavePosition.y.mul(this.rotationElevationMultiplier),
        0,
      );

      const rotationZ = sin(leavePosition.x.mul(this.rotationFrequency)).mul(
        rotationMultiplier,
      );
      const rotationX = sin(leavePosition.z.mul(this.rotationFrequency)).mul(
        rotationMultiplier,
      );
    ...
    const update = Fn(() => {
      ...
      const pushSideways = vec3(vehicleDelta.x, 0, vehicleDelta.z)
        .normalize()
        .mul(this.pushSidewaysMultiplier);

      const pushVelocity = vec3(
        this.vehicleVelocity.x,
        0,
        this.vehicleVelocity.z,
      ).mul(this.pushMultiplier);
      ...
      const vehiclePush = pushVelocity
        .add(pushSideways)
        .mul(speedMultiplier)
        .mul(vehicleMultiplier); //.mul(inverseWeight)
      ...
      const explosionPush = explosionDirection
        .mul(explosionMultiplier)
        .mul(this.explosion.a);
      ...
      velocity.y = velocity.xz
        .length()
        .min(2)
        .mul(this.upwardMultiplier)
        .mul(upwardDim);

      // Damping
      const groundDamping = terrainData.b.remapClamp(
        0.4,
        0,
        this.waterDamping,
        this.defaultDamping,
      ...
      const damping = max(groundDamping, inTheAirDamping).mul(
        this.game.ticker.deltaScaledUniform,
      );
      ...
      const floorY = terrainData.b
        .remapClamp(0.02, 0.13, 0, this.game.water.surfaceElevationUniform)
        .add(0.02);
      ...
      position.x.assign(
        mod(position.x.add(halfSize).sub(this.focusPoint.x), this.size)
          .sub(halfSize)
          .add(this.focusPoint.x),
      );
      position.z.assign(
        mod(position.z.add(halfSize).sub(this.focusPoint.y), this.size)
          .sub(halfSize)
          .add(this.focusPoint.y),
      );
    })();
    ...
    if (this.game.debug.active) {
      ...
      this.debugPanel.addBinding(this.scale, "value", {
        label: "scale",
        min: 0,
        max: 1,
        step: 0.001,
      ...
      this.debugPanel.addBinding(this.gravity, "value", {
        label: "gravity",
        min: 0,
        max: 20,
        step: 0.01,
  ...
  update() {
    this.focusPoint.value.set(
      this.game.view.optimalArea.position.x,
      this.game.view.optimalArea.position.z,
    );
