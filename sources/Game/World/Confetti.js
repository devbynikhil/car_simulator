...
export class Confetti {
  constructor() {
    ...
    if (this.game.debug.active) {
      this.game.debug.addThreeColorBinding(
        this.debugPanel,
        this.colorsUniform.array[0],
        "color0",
      );
      this.game.debug.addThreeColorBinding(
        this.debugPanel,
        this.colorsUniform.array[1],
        "color1",
      );
      this.game.debug.addThreeColorBinding(
        this.debugPanel,
        this.colorsUniform.array[2],
        "color2",
      );
  ...
  createPool() {
    for (let i = 0; i < this.poolSize; i++) {
      ...
      const randomProgressBuffer = instancedArray(
        randomProgressArray,
        "float",
      ).toAttribute();
      ...
      material.positionNode = Fn(() => {
        ...
        const y = progress
          ...
          .oneMinus()
          .mul(strength)
          .mul(confetti.elevationUniform);

        const finalPosition = vec3(
          basePosition.x.add(x),
          basePosition.y.add(y),
          basePosition.z.add(z),
        );
      ...
      confetti.mesh = new THREE.InstancedMesh(
        this.geometry,
        material,
        this.count,
      );
  ...
  pop(position = null, radius = 4, elevation = 6) {
    ...
    if (availableConfetti) availableConfetti.pop(_position, radius, elevation);
