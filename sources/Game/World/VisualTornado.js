...
export class VisualTornado {
  ...
  setMesh() {
    ...
    material.outputNode = Fn(() => {
      const scaledTime = this.game.ticker.elapsedScaledUniform
        .mul(timeScale)
        .negate();
       ...
       */
      // Noise 1
      const emissiveNoise1Uv = uv().add(
        vec2(scaledTime.mul(1.1), scaledTime.mul(1.1)),
      );
      emissiveNoise1Uv.assign(
        skewedUv(emissiveNoise1Uv, vec2(-1, 0)).mul(vec2(4, 0.5)),
      );
      const emissiveNoise1 = texture(
      ...
      // Noise 2
      const emissiveNoise2Uv = uv().add(
        vec2(scaledTime.mul(0.7), scaledTime.mul(0.7)),
      );
      emissiveNoise2Uv.assign(
        skewedUv(emissiveNoise2Uv, vec2(-1, 0)).mul(vec2(10, 2)),
      );
      const emissiveNoise2 = texture(
        this.game.noises.perlin,
        emissiveNoise2Uv,
        1,
      ).r.remap(0.45, 0.7);
      ...
      // Final noise
      const emissiveNoise = emissiveNoise1
        .mul(emissiveNoise2)
        .add(heightModifier)
    ...
    // Debug
    if (this.game.debug.active) {
      this.debugPanel.addBinding(this.visibility, "value", {
        label: "visibility",
        min: 0,
        max: 1,
        step: 0.001,
      ...
      this.debugPanel.addBinding(parabolOffset, "value", {
        label: "parabolOffset",
        min: 0,
        max: 4,
        step: 0.001,
