...
export class ExplosiveCrates {
    ...
    this.game.ticker.events.on(
      ...
      () => {
        for (const crate of this.items) {
          if (
            !crate.object.physical.body.isSleeping() &&
            crate.object.physical.body.isEnabled()
          )
  ...
  explode(crate) {
    ...
    gsap.delayedCall(0.4, () => {
      // Sound
      this.sounds.explosions[
        Math.floor(Math.random() * this.sounds.explosions.length)
      ].play(crate.reference.position);

      // Explode
      this.game.world.fireballs.create(
        crate.object.physical.body.translation(),
      );
