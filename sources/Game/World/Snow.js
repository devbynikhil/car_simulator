...
export class Snow {
  constructor() {
    ...
    this.achievementAchieved =
      this.game.achievements.groups.get("weatherSnow")?.items[0].achieved;
  ...
  setNodes() {
    ...
    const meltRatio = remapClamp(
      this.game.weather.temperature.value,
      0,
      10,
      0,
      -1,
    ...
    this.elevationNode = Fn(([position]) => {
      ...
      elevation.addAssign(
        noise1.mul(noise2).smoothstep(0, 1).mul(this.noiseMultiplier),
      );
      ...
      const chassisTracksHeight = groundDataColor.g
        .oneMinus()
        .remapClamp(0.5, 1, 0.25, 1);
    ...
    this.elevationBinding = this.game.debug.addManualBinding(
      ...
      () => {
        const elevationStrength =
          this.game.weather.snow.value *
          Math.max(this.game.dayCycles.progressDelta, 0) *
          10;
  ...
  setGeometry() {
    const positionArray = new Float32Array(
      this.subdivisions * this.subdivisions * 3 * 3 * 2,
    );
    const pivotArray = new Float32Array(
      this.subdivisions * this.subdivisions * 2 * 3 * 2,
    );
    ...
    for (let xIndex = 0; xIndex < this.subdivisions; xIndex++) {
      for (let zIndex = 0; zIndex < this.subdivisions; zIndex++) {
        ...
        const pivotX =
          (xIndex / (this.subdivisions - 1) - 0.5) *
          (this.size - this.subdivisionSize);
        const pivotZ =
          (zIndex / (this.subdivisions - 1) - 0.5) *
          (this.size - this.subdivisionSize);
    ...
    this.geometry.setAttribute(
      "pivot",
      new THREE.BufferAttribute(pivotArray, 2),
    );
  }
  ...
  setMaterial() {
    ...
    this.material.positionNode = Fn(() => {
      ...
      const cornerA = pivotCenter.add(
        vec3(-this.subdivisionSize, 0, -this.subdivisionSize),
      );
      const cornerB = pivotCenter.add(
        vec3(this.subdivisionSize, 0, -this.subdivisionSize),
      );
      const cornerC = pivotCenter.add(
        vec3(this.subdivisionSize, 0, this.subdivisionSize),
      );
      const cornerD = pivotCenter.add(
        vec3(-this.subdivisionSize, 0, this.subdivisionSize),
      );
      ...
      const positionC = positionA.add(
        vec3(0, 0, this.normalNeighbourShift.negate()),
      );
      ...
      // Normal
      const newNormal = cross(
        positionA.sub(positionB),
        positionA.sub(positionC),
      ).normalize();
    ...
    this.material.outputNode = Fn(() => {
      ...
      const hash = texture(this.game.noises.hash, hashUv)
        .r.mul(2)
        .add(this.glitterVariation)
        .mod(2)
        .sub(1)
        .abs();
    ...
    if (this.game.debug.active) {
      ...
      this.debugPanel.addBinding(this, "glitterViewMultiplier", {
        min: 0,
        max: 0.001,
        step: 0.000001,
      });
      this.debugPanel.addBinding(this, "glitterTimeMultiplier", {
        min: 0,
        max: 0.001,
        step: 0.000001,
      });
  ...
  update() {
    ...
    if (this.elevation.value > -0.9) {
      ...
      this.roundedPosition.value.x =
        Math.round(
          this.game.view.optimalArea.position.x / this.subdivisionSize,
        ) * this.subdivisionSize;
      this.roundedPosition.value.y =
        Math.round(
          this.game.view.optimalArea.position.z / this.subdivisionSize,
        ) * this.subdivisionSize;
      ...
      // Render
      const rendererState = THREE.RendererUtils.resetRendererState(
        this.game.rendering.renderer,
      );

      this.game.rendering.renderer.setPixelRatio(1);
      this.game.rendering.renderer.setRenderTarget(
        this.snowElevation.renderTarget,
      );
      this.snowElevation.quadMesh.render(this.game.rendering.renderer);
      this.game.rendering.renderer.setRenderTarget(null);
