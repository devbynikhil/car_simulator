...
export class LabArea extends Area {
  ...
  setImages() {
    ...
    this.images.update = () => {
      ...
      this.images.animationDirection.value =
        this.navigation.direction === LabArea.DIRECTION_NEXT ? 1 : -1;
    };
  }
  ...
  setScroller() {
      ...
      for (const project of labData) {
        ...
        // Image
        {
          ...
          mini.startLoading = () => {
            ...
            loader.load(`lab/images/${project.imageMini}`, (loadedTexture) => {
    ...
    this.scroller.animate = () => {
      ...
      this.scroller.chainLeft.position.y =
        -this.scroller.repeatAmplitude * 0.5 -
        (this.scroller.offset % this.scroller.repeatAmplitude);
      ...
      this.scroller.gearB.rotation.x =
        -this.scroller.gearA.rotation.x * (6 / 12);
      this.scroller.gearC.rotation.x =
        -this.scroller.gearB.rotation.x * (6 / 12);

      for (const mini of this.scroller.minis.items) {
        mini.group.position.y =
          safeMod(mini.y - this.scroller.offset, this.scroller.minis.total) - 1;
    ...
    this.scroller.update = () => {
      ...
      const closestProgress =
        Math.round(
          (this.scroller.progress + this.navigation.index - centeringOffset) /
            labData.length,
        ) *
          labData.length -
    ...
    this.game.inputs.events.on("labScroll", (action) => {
      this.scroller.targetProgress -=
        action.value * this.scroller.wheelSensitivity;
    });
  }
  ...
  setCandleFlames() {
    ...
    const baseMaterial = this.game.materials.getFromName(
    ...
    material.positionNode = Fn(() => {
      ...
      const wave = sin(
        this.game.ticker.elapsedScaledUniform.mul(0.3).add(uv().y.mul(3)),
      );
  ...
  changeProject(index = 0, direction = null, silent = false) {
    ...
    this.navigation.next =
      labData[(this.navigation.index + 1) % labData.length];
