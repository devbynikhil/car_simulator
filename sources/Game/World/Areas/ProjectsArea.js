...
export class ProjectsArea extends Area {
  ...
  constructor(model) {
    ...
    if (this.game.debug.active) {
      this.debugPanel
        .addButton({ title: "open", label: "open" })
        .on("click", () => {
          this.open();
        });
      this.debugPanel
        .addButton({ title: "close", label: "close" })
        .on("click", () => {
          this.close();
        });
    }
  ...
  setCinematic() {
    ...
    const applyPositionAndTarget = () => {
      const flatPosition = this.references.items
        .get("interactivePoint")[0]
        .position.clone();
      flatPosition.y = 0;
      this.cinematic.position
        .copy(flatPosition)
        .add(this.cinematic.positionOffset);
    ...
    if (this.game.debug.active) {
      ...
      debugPanel
        .addBinding(this.cinematic.positionOffset, "x", {
          label: "positionX",
          min: -10,
          max: 10,
          step: 0.05,
      ...
      debugPanel
        .addBinding(this.cinematic.positionOffset, "y", {
          label: "positionY",
          min: 0,
          max: 10,
          step: 0.05,
        ...
        .on("change", applyPositionAndTarget);
      debugPanel
        .addBinding(this.cinematic.positionOffset, "z", {
          label: "positionZ",
          min: -10,
          max: 10,
          step: 0.05,
        ...
        .on("change", applyPositionAndTarget);
      debugPanel
        .addBinding(this.cinematic.targetOffset, "x", {
          label: "targetX",
          min: -10,
          max: 10,
          step: 0.05,
        ...
        .on("change", applyPositionAndTarget);
      debugPanel
        .addBinding(this.cinematic.targetOffset, "y", {
          label: "targetY",
          min: 0,
          max: 10,
          step: 0.05,
        ...
        .on("change", applyPositionAndTarget);
      debugPanel
        .addBinding(this.cinematic.targetOffset, "z", {
          label: "targetZ",
          min: -10,
          max: 10,
          step: 0.05,
  ...
  setShadeMix() {
    ...
    if (this.game.debug.active) {
      ...
      debugPanel
        .addBinding(this.shadeMix.images, "min", {
          label: "imagesMin",
          min: 0,
          max: 1,
          step: 0.001,
      ...
      debugPanel
        .addBinding(this.shadeMix.images, "max", {
          label: "imagesMax",
          min: 0,
          max: 1,
          step: 0.001,
        ...
        .on("change", debugUpdate);
      debugPanel
        .addBinding(this.shadeMix.texts, "min", {
          label: "textsMin",
          min: 0,
          max: 1,
          step: 0.001,
        ...
        .on("change", debugUpdate);
      debugPanel
        .addBinding(this.shadeMix.texts, "max", {
          label: "textsMax",
          min: 0,
          max: 1,
          step: 0.001,
  ...
  setHover() {
    ...
    // Active
    this.hover.activeMaterial = new THREE.MeshBasicNodeMaterial({
      transparent: true,
    });
    this.hover.activeMaterial.outputNode = vec4(
      this.hover.baseColor.mul(1.5),
      float(1),
    );
  }
  ...
  setImages() {
    ...
    this.images.init = (key) => {
      ...
      const colorNode = Fn(() => {
        ...
        uvNew.x.addAssign(
          this.images.animationProgress
            .oneMinus()
            .mul(-0.25)
            .mul(this.images.animationDirection),
        );
        uvOld.x.addAssign(
          this.images.animationProgress
            .mul(0.25)
        ...
        // Load mix
        textureNewColor.assign(
          mix(color("#333333"), textureNewColor, this.images.loadProgress),
        );
      ...
      this.images.material.outputNode = Fn(() => {
        return vec4(
          mix(baseOutput.rgb, colorNode, this.shadeMix.images.mixUniform),
          baseOutput.a,
        );
      })();
    ...
    this.images.loadEnded = (key) => {
      ...
      if (this.navigation.current.images[this.images.index] === key) {
        ...
        gsap.to(this.images.loadProgress, {
          value: 1,
          duration: 1,
          overwrite: true,
        });
    ...
    this.images.update = (direction) => {
      ...
      this.images.animationDirection.value =
        direction === ProjectsArea.DIRECTION_NEXT ? 1 : -1;
    };
  }
  ...
  setPagination() {
    ...
    this.pagination.group =
      this.references.items.get("pagination")[0].children[0];
    this.pagination.items = [];

    // List
    let i = 0;
    const intersectPagination = this.references.items.get(
      "intersectPagination",
    );
    ...
    for (const child of this.pagination.group.children) {
      if (child instanceof THREE.Mesh) {
        ...
        ((item.intersect = this.game.rayCursor.addIntersect({
          active: false,
          shape: new THREE.Sphere(
            new THREE.Vector3(),
            item.intersectReference.scale.x,
          ),
          onClick: () => {
          ...
          item.intersectReference.getWorldPosition(
            item.intersect.shape.center,
          ));
    ...
    // Adjacents
    const intersectPrevious = this.references.items.get(
      "intersectPreviousImage",
    )[0];
    ...
    this.pagination.previousIntersect = this.game.rayCursor.addIntersect({
      active: false,
      shape: new THREE.Sphere(
        intersectPreviousPosition,
        intersectPrevious.scale.x,
      ),
      onClick: () => {
        this.previous();
      },
    ...
    this.pagination.update = () => {
      ...
      const offset =
        (-(this.navigation.current.images.length - 1) * this.pagination.inter) /
        2;
  ...
  setAttributes() {
    ...
    for (const child of this.attributes.group.children) {
      ...
      const textMesh = item.group.children.find((_child) =>
    ...
    this.attributes.update = () => {
      ...
      gsap.delayedCall(1, () => {
        ...
        this.attributes.group.position.y =
          this.attributes.originalY + ((i - 1) * 0.75) / 2;
      });
    };
  }
  ...
  setAdjacents() {
    ...
    // Text
    this.adjacents.previous.textMesh =
      this.adjacents.previous.inner.children.find((_child) =>
        _child.name.startsWith("text"),
      );
    ...
    this.texts.createMaterialOnMesh(
      this.adjacents.previous.textMesh,
      this.adjacents.previous.textCanvas.texture,
    );
    ...
    // Text
    this.adjacents.next.textMesh = this.adjacents.next.inner.children.find(
      (_child) => _child.name.startsWith("text"),
    );
    ...
    this.texts.createMaterialOnMesh(
      this.adjacents.next.textMesh,
      this.adjacents.next.textCanvas.texture,
    );
    ...
    this.adjacents.update = () => {
      ...
      gsap.delayedCall(1, () => {
        ...
        this.adjacents.previous.textCanvas.updateText(
          this.navigation.previous.titleSmall,
        );
        this.adjacents.next.textCanvas.updateText(
          this.navigation.next.titleSmall,
        );
      });
    };
  }
  ...
  setTitle() {
    ...
    this.title.textMesh = this.title.inner.children.find((_child) =>
      _child.name.startsWith("text"),
    );
  ...
  setUrl() {
    ...
    // Text
    this.url.textMesh = this.url.inner.children.find((_child) =>
      _child.name.startsWith("text"),
    );
    this.url.panel = this.url.inner.children.find((_child) =>
      _child.name.startsWith("panel"),
    );
    ...
    this.url.update = (direction) => {
      ...
      gsap.to(this.url.inner.rotation, {
        ...
        onComplete: () => {
          ...
          const ratio =
            this.url.textCanvas.getMeasure().width / this.texts.density;
  ...
  setDistinctions() {
    ...
    this.distinctions.items.awwwards = this.distinctions.group.children.find(
      (_child) => _child.name.startsWith("awwwards"),
    );
    this.distinctions.items.fwa = this.distinctions.group.children.find(
      (_child) => _child.name.startsWith("fwa"),
    );
    this.distinctions.items.cssda = this.distinctions.group.children.find(
      (_child) => _child.name.startsWith("cssda"),
    );
    ...
    this.distinctions.update = () => {
      ...
      gsap.delayedCall(1, () => {
        ...
        const positions =
          this.distinctions.positions[
            this.navigation.current.distinctions.length - 1
          ];
  ...
  setBlackBoard() {
    ...
    this.blackBoard.timeline.to(
      this.blackBoard.group.position,
      { y: 0, ease: "power2.in", duration: 0.7 },
      0.7 + 2,
    );

    this.blackBoard.timeline.to(
      this.blackBoard.group.rotation,
      { x: 0.1, duration: 0.15 },
      0 + 2,
    );
    this.blackBoard.timeline.to(
    ...
    // Labels
    this.blackBoard.labelsGamepadPlaystation = this.references.items.get(
      "blackboardLabelsGamepadPlaystation",
    )[0];
    this.blackBoard.labelsGamepadXbox = this.references.items.get(
      "blackboardLabelsGamepadXbox",
    )[0];
    this.blackBoard.labelsMouseKeyboard = this.references.items.get(
  ...
  setAnvil() {
    ...
    material.outputNode = Fn(() => {
      ...
      const emissiveOutput = emissiveColor
        .div(luminance(emissiveColor))
        .mul(intensity);
  ...
  open() {
    ...
    // Achievements
    this.game.achievements.setProgress(
      "projects",
      this.navigation.current.title,
    );
  }
  ...
  previousProject(firstImage = false) {
    ...
    this.changeProject(
      ...
      ProjectsArea.DIRECTION_PREVIOUS,
      firstImage,
    );
  ...
  changeProject(
    index = 0,
    direction = ProjectsArea.DIRECTION_NEXT,
    firstImage = false,
    silent = false,
  ) {
    // Loop index
    ...
    this.navigation.previous =
      projectsData[
        this.navigation.index - 1 < 0
          ? projectsData.length - 1
          : this.navigation.index - 1
      ];
    this.navigation.next =
      projectsData[(this.navigation.index + 1) % projectsData.length];
    ...
    if (firstImage) imageIndex = 0;
      ...
      imageIndex =
        direction === ProjectsArea.DIRECTION_NEXT
          ? 0
          : this.navigation.current.images.length - 1;
  ...
  changeImage(imageIndex = 0, direction = null, silent = false) {
    if (direction === null)
      direction =
        imageIndex > this.images.index
          ? ProjectsArea.DIRECTION_NEXT
          : ProjectsArea.DIRECTION_PREVIOUS;
  ...
  update() {
    // Oven
    this.oven.blower.scale.y =
      Math.sin(this.game.ticker.elapsedScaled) * 0.2 + 0.8;
    ...
    // Anvil
    const time =
      this.game.ticker.elapsedScaled * this.anvil.frequency + Math.PI * 0.25;
    this.anvil.hammer.rotation.x =
      Math.pow(1 - Math.abs(Math.sin(time)), 5) - 1;
