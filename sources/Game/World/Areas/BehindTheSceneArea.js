...
export class BehindTheSceneArea extends Area {
  ...
  setSlabs() {
    ...
    material.positionNode = Fn(() => {
      ...
      const osciliation = sin(
        this.game.ticker.elapsedScaledUniform.mul(0.5).add(random.mul(PI2)),
      ).mul(0.5);

      const distanceToVehicle = center
        .sub(this.vehicleRelativePosition)
        .length();
  ...
  update() {
    this.vehicleRelativePosition.value.x =
      this.game.physicalVehicle.position.x - this.slabs.position.x;
    this.vehicleRelativePosition.value.y =
      this.game.physicalVehicle.position.z - this.slabs.position.z;
