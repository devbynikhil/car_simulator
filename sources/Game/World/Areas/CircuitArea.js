...
export class CircuitArea extends Area {
  ...
  constructor(model) {
    ...
    this.game.materials.getFromName("circuitBrand").map.minFilter =
      THREE.LinearFilter;
    this.game.materials.getFromName("circuitBrand").map.magFilter =
      THREE.LinearFilter;
  }
  ...
  setStartingLights() {
    ...
    this.startingLights.redMaterial = this.game.materials.getFromName(
      "emissiveOrangeRadialGradient",
    );
    this.startingLights.greenMaterial = this.game.materials.getFromName(
  ...
  setTimer() {
    ...
    // Digits
    {
      ...
      context.fillText(
        "00:00:000",
        this.timer.digits.width * 0.5,
        this.timer.digits.height * 0.5,
      );
    ...
    this.timer.write = (text) => {
      ...
      this.timer.digits.context.fillRect(
        0,
        0,
        this.timer.digits.width,
        this.timer.digits.height,
      );

      this.timer.digits.context.fillStyle = "#ffffff";
      this.timer.digits.context.fillText(
        text,
        this.timer.digits.width * 0.5,
        this.timer.digits.height * 0.5,
      );
    ...
    this.timer.update = () => {
      ...
      if (this.timer.running) {
        this.timer.elapsedTime =
          this.game.ticker.elapsed - this.timer.startTime;
  ...
  setCheckpoints() {
    ...
    // Create checkpoints
    const baseCheckpoints = this.references.items
      .get("checkpoints")
    ...
    for (const baseCheckpoint of baseCheckpoints) {
      ...
      // Center
      checkpoint.center = new THREE.Vector2(
        checkpoint.position.x,
        checkpoint.position.z,
      );

      // Segment
      checkpoint.a = new THREE.Vector2(
        checkpoint.position.x - checkpoint.scale,
        checkpoint.position.z,
      );
      checkpoint.b = new THREE.Vector2(
        checkpoint.position.x + checkpoint.scale,
        baseCheckpoint.position.z,
      );
      ...
      checkpoint.reach = () => {
        ...
        if (this.game.world.confetti) {
          this.game.world.confetti.pop(
            new THREE.Vector3(checkpoint.a.x, 0, checkpoint.a.y),
          );
          this.game.world.confetti.pop(
            new THREE.Vector3(checkpoint.b.x, 0, checkpoint.b.y),
          );
        }
        ...
        // Next checkpoint
          ...
          const newTarget =
            this.checkpoints.items[
              this.checkpoints.reachedCount % (this.checkpoints.count + 1)
            ];
          newTarget.setTarget();
        }
    ...
    if (this.game.debug.active) {
      ...
      this.game.debug.addThreeColorBinding(
        debugPanel,
        this.checkpoints.doorTarget.color.value,
        "targetColor",
      );
      this.game.debug.addThreeColorBinding(
        debugPanel,
        this.checkpoints.doorReached.color.value,
        "reachedColor",
      );

      debugPanel.addBinding(doorIntensity, "value", {
        label: "intensity",
        min: 0,
        max: 5,
        step: 0.01,
  ...
  setRoad() {
    this.roadBody =
      this.references.items.get("road")[0].userData.object.physical.body;
  ...
  setAirDancers() {
    ...
    material.normalNode = Fn(() => {
      ...
      newNormalGeometry.xy.assign(
        rotateUV(newNormalGeometry.xy, rotation, vec2(0)),
      );
  ...
  setResetTime() {
    ...
    material.outputNode = Fn(() => {
      ...
      return vec4(
        mix(baseOutput.rgb, color("#ffffff").mul(1.3), text),
        baseOutput.a,
      );
    })();
    ...
    this.resetTime.tryDraw = () => {
      const timeToReset =
        dayDuration - ((Date.now() - this.resetTime.resetTime) % dayDuration);

      const formatedTime = timeToReadableString(
        timeToReset / 1000,
        true,
        true,
        false,
      ...
      if (formatedTime !== this.resetTime.lastTimeDrawn) {
        ...
        this.resetTime.finalFormatedTime =
          formatedTime === "" ? "now" : `in ${formatedTime}`;
  ...
  setPodium() {
    ...
    this.podium.confettiPositionA = this.references.items
      .get("podiumConfettiA")[0]
      .position.clone();
    this.podium.confettiPositionB = this.references.items
      .get("podiumConfettiB")[0]
      .position.clone();
    ...
    this.podium.popConfetti = () => {
      ...
      this.game.world.confetti.pop(
        this.podium.confettiIndex % 2 === 0
          ? this.podium.confettiPositionA
          : this.podium.confettiPositionB,
      );
  ...
  setMenu() {
    ...
    this.menu.resetTimeElement =
      this.menu.instance.contentElement.querySelector(".js-reset-time");
    this.menu.leaderboardContainerElement =
      this.menu.instance.contentElement.querySelector(
        ".js-leaderboard-container",
      );
    this.menu.leaderboardElement =
      this.menu.leaderboardContainerElement.querySelector(
        ".js-leaderboard tbody",
      );
    this.menu.racingButtons =
      this.menu.instance.contentElement.querySelector(".js-racing-buttons");
    ...
    this.menu.updateLeaderboard = (scores = null) => {
      ...
      // Menu open => Update content
        ...
        if (scores.length)
          this.menu.leaderboardContainerElement.classList.remove(
            "has-no-score",
          );
        else
    ...
    const restartElement =
      this.menu.instance.contentElement.querySelector(".js-button-restart");
    ...
    // End button
    const endElement =
      this.menu.instance.contentElement.querySelector(".js-button-end");
    ...
    // Controls button
    const controlsElement = this.menu.instance.contentElement.querySelector(
      ".js-button-controls",
    );
  ...
  setEndModal() {
    ...
    this.endModal.timeElement =
      this.endModal.instance.element.querySelector(".js-time");

    // Restart button
    const restartElement =
      this.endModal.instance.element.querySelector(".js-button-restart");
    restartElement.addEventListener("click", (event) => {
      event.preventDefault();
    ...
    this.menu.inputGroup =
      this.endModal.instance.element.querySelector(".js-input-group");
  ...
  update() {
    if (this.state === CircuitArea.STATE_RUNNING) {
      ...
      for (const obstacle of this.obstacles.items) {
        ...
        const osciliation =
          Math.sin(this.timer.elapsedTime * 1.25 + obstacle.osciliationOffset) *
          5;
    ...
    for (const banner of this.banners) {
      ...
      const rotation =
        Math.sin(time) +
        Math.sin(time * 2.34) * 0.5 +
        Math.sin(time * 3.45) * 0.25;
