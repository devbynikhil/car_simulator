...
export class Floor {
  ...
  setBedRock() {
    ...
    this.bedRock.physical = this.game.physics.getPhysical({
      ...
      position: new THREE.Vector3(
        0,
        this.game.water.depthElevation - this.bedRock.halfHeight,
        0,
      ),
      frictionRule: "min",
      friction: 0.5,
      ...
      colliders: [
        {
          shape: "cuboid",
          parameters: [
            this.bedRock.halfWidth,
            this.bedRock.halfHeight,
            this.bedRock.halfWidth,
          ],
        },
      ],
  ...
  update() {
    this.mesh.position.x =
      Math.round(this.game.view.optimalArea.position.x / this.cellSize) *
      this.cellSize;
    this.mesh.position.z =
      Math.round(this.game.view.optimalArea.position.z / this.cellSize) *
      this.cellSize;

    // Bedrock
    if (
      Math.abs(this.game.player.position.x) >
        this.game.terrain.size / 2 - this.bedRock.halfWidth ||
      Math.abs(this.game.player.position.z) >
        this.game.terrain.size / 2 - this.bedRock.halfWidth
    ) {
