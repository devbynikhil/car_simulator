...
export class Scenery {
  constructor() {
    ...
    for (const child of model) {
      // Add
      if (
        typeof child.userData.prevent === "undefined" ||
        child.userData.prevent === false
      ) {
        // Objects
  ...
  setRoad() {
    ...
    const colorNode = Fn(() => {
      ...
      const hash = texture(this.game.noises.hash, hashUv)
        .r.mul(2)
        .add(this.road.glitterVariation)
        .mod(2)
        .sub(1)
        .abs();
    ...
    if (this.game.debug.active) {
      ...
      debugPanel.addBinding(this.road.glitterIntensity, "value", {
        label: "glitterIntensity",
        min: 0,
        max: 10,
        step: 0.01,
      });
      debugPanel.addBinding(this.road.glitterPerlinFrequency, "value", {
        label: "glitterPerlinFrequency",
        min: 0,
        max: 0.1,
        step: 0.0001,
      });
      debugPanel.addBinding(this.road.glitterHashFrequency, "value", {
  ...
  update() {
    this.road.glitterVariation.value +=
      this.game.ticker.deltaScaled * 0.004 +
      this.game.view.delta.length() * 0.004;
  }
}
