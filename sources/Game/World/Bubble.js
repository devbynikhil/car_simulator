...
export class Bubble {
  ...
  setMessage() {
    ...
    material.outputNode = Fn(() => {
      ...
      // Fog
      const foggedColor = this.game.fog.strength.mix(
        baseColor,
        this.game.fog.color,
      );
  ...
  updateText(text = "") {
    ...
    this.textWidth = Math.min(
      Math.ceil(textSize.width) + this.textPaddingHorizontal * 2 + 2,
      this.width,
    );
    ...
    this.context.fillText(
      text,
      this.textPaddingHorizontal + 1,
      this.height * 0.5 + this.textOffsetVertical,
    );
