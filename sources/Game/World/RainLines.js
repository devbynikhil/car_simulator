...
export class RainLines {
  constructor() {
    ...
    this.achievementAchieved =
      this.game.achievements.groups.get("weatherRain")?.items[0].achieved;
    ...
    if (this.game.debug.active) {
      ...
      this.debugPanel.addBinding(this.elevation, "value", {
        label: "elevation",
        min: 0,
        max: 50,
        step: 0.1,
      });
      this.debugPanel.addBinding(this.thickness, "value", {
        label: "thickness",
        min: 0,
        max: 0.1,
        step: 0.001,
    ...
    this.lengthRatioBinding = this.game.debug.addManualBinding(
      ...
      () => {
        ...
        const snowRatio =
          1 - Math.pow(1 - Math.max(this.game.weather.snow.value, 0), 4);
  ...
  setGeometry() {
    ...
    this.geometry.setAttribute(
      "offset",
      new THREE.Float32BufferAttribute(offsetArray, 2),
    );
    this.geometry.setAttribute(
      "random",
  ...
  setMaterial() {
    ...
    this.material.positionNode = Fn(() => {
      ...
      newPosition.x.assign(
        mod(newPosition.x.add(halfSize), this.size).sub(halfSize),
      );
      newPosition.z.assign(
        mod(newPosition.z.add(halfSize), this.size).sub(halfSize),
      );
      ...
      // Incline
      newPosition.xz.addAssign(
        tangent.mul(newPosition.y.mul(this.incline).mul(-1)),
      );
