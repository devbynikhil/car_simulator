...
export class Foliage {
    ...
    const alphaNode = Fn(() => {
      ...
      if (this.seeThrough) {
        ...
        alpha.assign(
          foliageSDF.mul(
            distanceFade
              .mul(this.material.threshold.oneMinus())
              .add(this.material.threshold),
          ),
        );
      } else {
        // Alpha
  ...
  setInstancedMesh() {
    ...
    this.instanceMatrix = new THREE.InstancedBufferAttribute(
      new Float32Array(this.mesh.count * 16),
      16,
    );
  ...
  update() {
    this.material.seeThroughPosition.value.copy(
      this.game.world.visualVehicle.screenPosition,
    );
    ...
    this.material.seeThroughEdgeMax.value =
      (15 / this.game.view.spherical.radius.current) *
      this.seeThroughMultiplier;
  }
}
