...
import MeshGridMaterial, {
  MeshGridMaterialLine,
} from "../Materials/MeshGridMaterial.js";
  ...
  setVisual() {
    ...
    uvGridMaterial.outputNode = Fn(() => {
      const distanceToCenter = positionWorld.xz
        .sub(this.game.reveal.position2Uniform)
        .length();
    ...
    // Debug
    if (this.game.debug.active) {
      this.debugPanel.addBinding(uvGridMaterial, "scale", {
        min: 0,
        max: 0.002,
        step: 0.0001,
      });
      ...
      for (const line of lines) {
        ...
        lineDebugPanel.addBinding(line.scale, "value", {
          label: "scale",
          min: 0,
          max: 1,
          step: 0.001,
        });
        lineDebugPanel.addBinding(line.thickness, "value", {
          label: "thickness",
          min: 0,
          max: 1,
          step: 0.001,
        });
        lineDebugPanel.addBinding(line.offset, "value", {
          label: "offset",
          min: 0,
          max: 1,
          step: 0.001,
        });
        lineDebugPanel.addBinding(line.cross, "value", {
          label: "cross",
          min: 0,
          max: 1,
          step: 0.001,
        ...
        lineDebugPanel
          .addBinding(
            {
              color: "#" + line.color.value.getHexString(THREE.SRGBColorSpace),
            },
            "color",
          )
