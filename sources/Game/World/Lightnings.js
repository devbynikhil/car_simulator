...
export class Lightnings {
  constructor() {
    ...
    if (this.game.debug.active) {
      this.debugPanel.addBinding(this, "frequency", {
        min: 0.1,
        max: 10,
        step: 0.1,
      });
      ...
      this.debugPanel.addBinding(this.intensity, "value", {
        label: "intensity",
        min: 0,
        max: 10,
        step: 0.01,
        ...
        this.sounds.near.push(
          this.game.audio.register({
            ...
            onPlay: (item, coordinates) => {
              ...
              const distanceRateEffect = mathRemapClamp(
                distance,
                0,
                20,
                0,
  ...
  setAnticipationParticles() {
    ...
    const positionArray = new Float32Array(
      this.anticipationParticles.count * 3,
    );
    ...
    this.anticipationParticles.positionAttribute = instancedArray(
      positionArray,
      "vec3",
    ).toAttribute();
    this.anticipationParticles.scaleAttribute = instancedArray(
      scaleArray,
      "float",
    ).toAttribute();
    ...
    this.anticipationParticles.positionNode = Fn(([_startTime]) => {
      ...
      const timeProgress = min(
        localTime.div(this.anticipationParticles.duration),
        1,
      );
    ...
    this.anticipationParticles.create = (coordinates, rng) => {
      ...
      material.colorNode = this.colorB
        .div(luminance(this.colorB))
        .mul(this.intensity);
      material.positionNode =
        this.anticipationParticles.positionNode(startTime);
      material.scaleNode = this.anticipationParticles.scaleNode(startTime);

      const mesh = new THREE.Mesh(
        this.anticipationParticles.geometry,
        material,
      );
    ...
    if (this.game.debug.active) {
      ...
      debugPanel
        .addBinding(this.anticipationParticles, "duration", {
          min: 0,
          max: 10,
          step: 0.01,
        })
      ...
      debugPanel.addBinding(scaleUniform, "value", {
        label: "scale",
        min: 0,
        max: 1,
        step: 0.001,
      });
      debugPanel.addBinding(elevationUniform, "value", {
        label: "elevation",
        min: 0,
        max: 5,
        step: 0.01,
  ...
  setArc() {
    ...
    this.arc.vertexNode = Fn(([_startTime]) => {
      ...
      const nextDelta = nextWorldPosition.xyz
        .sub(worldPosition.xyz)
        .normalize();
    ...
    this.arc.create = (coordinates, rng) => {
      ...
      material.colorNode = mixedColor
        .div(luminance(mixedColor))
        .mul(this.intensity);
      material.fog = false;
    ...
    if (this.game.debug.active) {
      ...
      debugPanel.addBinding(this.arc, "duration", {
        min: 0,
        max: 10,
        step: 0.01,
      });
      debugPanel.addBinding(easeOutUniform, "value", {
        label: "easeOut",
        min: 1,
        max: 10,
        step: 1,
      ...
      debugPanel.addBinding(thicknessUniform, "value", {
        label: "thickness",
        min: 0,
        max: 1,
        step: 0.001,
  ...
  setExplosionParticles() {
    ...
    this.explosionParticles.positionAttribute = instancedArray(
      positionArray,
      "vec3",
    ).toAttribute();
    this.explosionParticles.scaleAttribute = instancedArray(
      scaleArray,
      "float",
    ).toAttribute();
    ...
    this.explosionParticles.positionNode = Fn(([_startTime]) => {
      const localTime = this.game.ticker.elapsedScaledUniform.sub(_startTime);
      const timeProgress = min(
        localTime.div(float(this.explosionParticles.duration).mul(0.75)),
        1,
      );

      const newPosition = this.explosionParticles.positionAttribute.toVar();
      newPosition.mulAssign(
        timeProgress
          .oneMinus()
          .pow(easeOutUniform)
          .oneMinus()
    ...
    this.explosionParticles.scaleNode = Fn(([_startTime]) => {
      const localTime = this.game.ticker.elapsedScaledUniform.sub(_startTime);
      const timeScale = localTime
        .div(this.explosionParticles.duration)
        .oneMinus()
        .max(0);
      const finalScale = this.explosionParticles.scaleAttribute
        .mul(scaleUniform)
        .mul(timeScale);
      return finalScale;
    });
    ...
    if (this.game.debug.active) {
      ...
      debugPanel.addBinding(this.explosionParticles, "duration", {
        min: 0,
        max: 10,
        step: 0.01,
      });
      ...
      debugPanel.addBinding(radiusUniform, "value", {
        label: "radius",
        min: 0,
        max: 10,
        step: 0.001,
      });
      debugPanel.addBinding(this.explosionParticles, "fallAmplitude", {
        min: 0,
        max: 2,
        step: 0.001,
      });
    ...
    this.create(
      new THREE.Vector3(
        focusPointPosition.x +
          (rng() - 0.5) * this.game.view.optimalArea.radius * 1,
        0,
  ...
  create(coordinates, rng) {
    ...
    gsap.delayedCall(this.anticipationParticles.duration, () => {
      // Sound
      this.sounds.near[
        Math.floor(Math.random() * this.sounds.near.length)
      ].play(coordinates);
      ...
      const duration = Math.max(
        this.arc.duration,
        this.explosionParticles.duration,
      );
    ...
    if (currentSecond !== this.currentSecond) {
      ...
      // Distant thunder
        ...
        if (rng() < this.hitChances) {
          this.sounds.distant[
            Math.floor(Math.random() * this.sounds.distant.length)
          ].play();
        }
      }
    }
