  ...
  setLeaves() {
    ...
    for (const treeReference of this.references) {
      for (const leaves of this.modelParts.leaves) {
        const finalMatrix = leaves.matrix
          .clone()
          .premultiply(treeReference.matrixWorld);
    ...
    if (this.game.debug.active) {
      this.game.debug.addThreeColorBinding(
        this.debugPanel,
        leavesColorANode.value,
        "leavesColorA",
      );
      this.game.debug.addThreeColorBinding(
        this.debugPanel,
        leavesColorBNode.value,
        "leavesColorB",
      );
      ...
      this.debugPanel.addBinding(
        this.leaves.material.seeThroughEdgeMin,
        "value",
        { label: "seeThroughEdgeMin", min: 0, max: 1, step: 0.001 },
      );
      this.debugPanel.addBinding(
        this.leaves.material.seeThroughEdgeMax,
        "value",
        { label: "seeThroughEdgeMax", min: 0, max: 1, step: 0.001 },
      );
    }
  }
  ...
  setPhysical() {
    for (const treeReference of this.references) {
      this.game.objects.add(null, {
        ...
        sleeping: true,
        colliders: [
          { shape: "cylinder", parameters: [2.5, 0.15], category: "object" },
        ],
