...
export class PoleLights {
  constructor() {
    ...
    this.glass = this.instancedGroup.meshes.find(
      (mesh) => mesh.instance.name === "glass",
    ).instance;
  ...
  setPhysics() {
    for (const reference of this.references) {
      this.game.objects.add(null, {
        ...
        colliders: [
          { shape: "cuboid", parameters: [0.2, 1.7, 0.2], category: "object" },
        ],
  ...
  setFireflies() {
    ...
    material.outputNode = this.game.materials.getFromName(
      "emissiveOrangeRadialGradient",
    ).outputNode;

    const baseTime = this.game.ticker.elapsedScaledUniform.add(
      hash(instanceIndex).mul(999),
    );
