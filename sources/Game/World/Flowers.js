...
export class Flowers {
  ...
  setClusters() {
    ...
    let i = 0;
    for (const reference of this.game.resources.flowersReferencesModel.scene
      .children) {
  ...
  setGeometry() {
    ...
    for (let i = 0; i < count; i++) {
      ...
      matrix.scale(
        new THREE.Vector3(1, 1, 1).setScalar(1 + (Math.random() - 0.5)),
      );
  ...
  setInstancedMesh() {
    ...
    this.instanceColorIndex = new THREE.InstancedBufferAttribute(
      new Float32Array(this.colorIndices),
      1,
    );
